...
- **创建前必须查重**：调用任何 `save-*` / `add-*` 前，必须先调用 `check-duplicate --topic "主题" --kind <类型>` 确认是否已存在。如果存在且主题一致，使用 `update-*` 更新；如果存在但主题不同，创建新条目并在 topic 中备注区别。
- **学习计划查重**：用户想学习某个主题前，先调用 `list-active-plans` 查看是否有进行中的计划。如果有，继续现有计划而不是创建新计划。
- **授课记录查重**：开始教学前，先调用 `list-recent-lessons --topic "主题"` 查看最近是否教过。如果教过且用户未掌握，继续上次 lesson 而不是创建新 lesson。
...
- **创建前先遍历树**：创建新 knowledge 前，必须先通过 `tree-roots` → `tree-children` 逐层查找，确认该 topic 不在已有节点中。只有确认不存在时才创建新节点。
...
**飞书为主存储，本地为缓存。** 有 `data/feishu-config.json` 时自动双写，无需手动配置。
...
- **写入自动同步**：`save-knowledge`、`update-knowledge`、`complete-digest` 自动同步到飞书
- **读取自动拉取**：`get-knowledge` 本地不存在时自动从飞书拉取
- **增量更新**：已有条目内容变化时自动覆盖更新，无变化跳过
...
# 从飞书拉取全部节点到本地缓存
python ./feishu.py pull --all

# 拉取单个节点
python ./feishu.py pull --node-token "xxx"
...
# 知识库（先搜后存：必须先 search/list 确认不存在，再 save 创建）
python ./kb.py search --kind knowledge --query "主题"
python ./kb.py list-knowledge --topic "主题"
