...
- **查找优先于创建**：在调用任何 `save-*` / `add-*` 命令之前，必须先用 `search --kind <type> --query "关键词"` 或 `list-*` 系列命令检查是否已存在同类条目。如果找到匹配的已有条目，应使用 `update-*` 或传入已有 `--id` 进行更新复用，而不是创建新条目。只有确认不存在时才创建新的。
- **飞书优先于本地**：飞书是主存储，本地是缓存。创建任何条目前，必须先查询飞书是否已有同名/相似文档（`python ./feishu.py sync --dry-run` 或查看已有节点）。如果飞书已有且主题一致，应更新现有文档而不是创建新文档。如果碰巧同名但主题不同，创建新文档并备注区别。
